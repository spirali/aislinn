...
int main(int argc, char **argv)
{
	...
	if (root == rank) {
		for (int i = 0; i < mysize; i++) {
			out1[i] = d[rank * mysize + i] * 10;
		}
		MPI_Scatter(d, mysize, MPI_INT, MPI_IN_PLACE, mysize, MPI_INT, root, MPI_COMM_WORLD);
	} else {
	...
	if (root == rank) {
		for (int i = 0; i < mysize; i++) {
			out2[rank * mysize + i] = out1[i] * 10;
		}
		MPI_Gather(MPI_IN_PLACE, 0, MPI_INT, out2, mysize, MPI_INT, root, MPI_COMM_WORLD);
	} else {
